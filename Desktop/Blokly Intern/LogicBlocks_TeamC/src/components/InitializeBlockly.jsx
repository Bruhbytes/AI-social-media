...
const InitializeBlockly = (toolboxXml) => {
  ...
  workspace.addChangeListener((event) => {
    if (
      ...
      // Get the generated code and handle it
      const code = Blockly.JavaScript.workspaceToCode
        ? Blockly.JavaScript.workspaceToCode(workspace)
        : workspace.toJavaScriptCode();

      handleGeneratedCode(code);
    }
  });

  const handleGeneratedCode = (code) => {
    // Handle the generated code here
    // You may dispatch actions, execute functions, etc.
    // Example: eval(code);
